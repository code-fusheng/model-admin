  ...
  <div v-loading="loading">
    ...
    <el-form :inline="true" :model="page" class="demo-form-inline" size="mini">
      <el-form-item label="模糊查询">
        <el-input v-model="page.params.collectionUserId" placeholder="用户名字段" clearable />
      </el-form-item>
      ...
      <el-form-item label="起始日期">
        <el-date-picker
          v-model="page.params.collectionTime"
          type="daterange"
          align="right"
          unlink-panels
          range-separator="至"
    ...
    >
      ...
      <el-table-column prop="collectionTime" label="收藏时间" min-width="220" align="center" sortable="custom" />
      <el-table-column prop="collectionType" label="收藏类型" min-width="150" align="center">
        <template slot-scope="scope">
          <el-tag v-if="scope.row.collectionType === 1">文章的收藏</el-tag>
...
<script>
// 导入api接口定义的方法 接收变量为 collectionApi
  ...
  methods: {
    ...
    deleteByIds() {
      ...
      this.$confirm('删除之后无法恢复，是否删除?', '提示', {
        ...
        const ids = []
        this.selectcollections.forEach(e => {
          ids.push(e.modelId)
        })
        collectionApi.deleteByIds(ids).then(res => {
    ...
    // 修改
    toUpdate(val) {
      collectionApi.get(val).then(res => {
    ...
    toEnable(id) {
      this.$confirm('是否启用？', '提示', {
        ...
        collectionApi.enable(id).then(res => {
          this.$message.success(res.msg)
          this.getByPage()
        })
      }).catch(() => {
    ...
    toDisable(id) {
      this.$confirm('是否弃用?', '提示', {
        ...
        collectionApi.disable(id).then(res => {
          this.$message.success(res.msg)
          this.getByPage()
        })
      }).catch(() => {
    ...
    // 恢复搜索框
    refresh() {
      this.page.params.collectionTime = null
      this.page.currentPage = 1
      this.page.params.collectionType = null
      this.page.params.collectionState = null
